...
class MCL(object):
    ...
    def __init__(self, NUM, mean, std):
        ...
        self.MAP = np.array([[0, 0], [3, 5], [5, -7], [-1, 8], [-8, -9], [-8, 8]], dtype=np.float)
    ...
    def calc_weights_plain(self, readings):
        readings = readings.reshape(-1, 2)
        self.weights = np.ones(self._NUM, dtype=np.float)
    ...
    def sample(self):
        ...
        indeces = []
        # print(type(self.weights), self.weights.shape)
        index = int(np.random.random() * self._NUM )
        beta = 0.0
        mw = max(self.weights)
        for i in np.arange(self._NUM):
            beta += np.random.random() * 2 * mw
            while(beta > self.weights[index]):
                beta -= self.weights[index]
                index = (index + 1) % self._NUM
